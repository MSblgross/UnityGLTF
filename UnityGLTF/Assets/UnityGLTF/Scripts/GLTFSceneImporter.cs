...
namespace UnityGLTF
{
	public class GLTFSceneImporter
	{
		...
		protected Func<GLTF.Schema.Material, int, UnityEngine.Material> _fMaterialLoadCallback;
		...
		public void SetMaterialCallback(Func<GLTF.Schema.Material, UnityEngine.Material> materialLoadCallback)
		{
		...
		protected IEnumerator ImportScene(int sceneIndex = -1, bool isMultithreaded = false)
		{
			...
			if (_lastLoadedScene == null)
			{
				...
				if (_gltfRoot.Buffers != null)
				{
					...
					{
						GLTF.Schema buffer = _root.Buffers[i];
		...
		protected virtual GameObject CreateMeshPrimitive(MeshPrimitive primitive, int meshID, int primitiveIndex)
		{
			...
			GLTF.Material materialToLoad = shouldUseDefaultMaterial ? DefaultMaterial : primitive.Material.Value;
		...
		protected virtual UnityEngine.Texture CreateTexture(GLTF.Schema.Texture texture)
		{
			if (_assetCache.TextureCache[texture.Source.Id] == null)
			{
				...
				{
					_assetCache.ImageCache[texture.Source.Id] = _loader.LoadImage(_gltfRoot.Images[texture.Source.Id]);
				}
