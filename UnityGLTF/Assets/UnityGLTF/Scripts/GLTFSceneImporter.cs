	...
	public class GLTFSceneImporter
	{
		...
		private IEnumerator ConstructBufferData(Node node)
		{
			...
			if (mesh != null)
			{
				...
				{
					yield return ConstructMeshCacheData(mesh.Value, mesh);
				}
			}
		...
		private IEnumerator ConstructMeshCacheData(GLTF.Schema.Mesh mesh, MeshId meshId)
		{
		...
		protected IEnumerator ConstructImage(GLTF.Schema.Image image, int imageCacheIndex, bool markGpuOnly = true, bool linear = true)
		{
		...
		protected virtual IEnumerator ConstructUnityTexture(Stream stream, bool markGpuOnly, bool linear, GLTF.Schema.Image image, int imageCacheIndex)
		{
		...
		protected virtual IEnumerator ConstructMeshAttributes(MeshPrimitive primitive, int meshID, int primitiveIndex)
		{
			if (_assetCache.MeshCache[meshID][primitiveIndex].MeshAttributes.Count == 0)
			{
				...
				GLTFHelpers.BuildMeshAttributes(ref attributeAccessors, SchemaExtensions.CoordinateSpaceConversionScale, SchemaExtensions.TextureSpaceConversionScale, SchemaExtensions.TangentSpaceConversionScale);
		...
		private BoneWeight[] CreateBoneWeightArray(Vector4[] joints, Vector4[] weights, uint vertCount)
		{
...
protected IEnumerator ConstructUnityMesh(MeshConstructionData meshConstructionData, int meshId, int primitiveIndex)
		{
			...
			uint vertexCount = primitive.Attributes[SemanticProperties.POSITION].Value.Count;
			...
			UnityEngine.Mesh mesh = new UnityEngine.Mesh
			{
				...
				triangles = primitive.Indices != null
					...
					: SchemaExtensions.GenerateTriangles(vertexCount),
